...
export function Projects() {
  return (
    <section id="projects" className="section-shell border-y border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
      <SectionHeading eyebrow="04 / Projects" title="A few things I’ve built." intro="Explore my movie and recipe discovery projects, with more work to share soon." />
        ...
        {portfolio.projects.map((project) => (
          <article key={project.title} className="project-card group">
            <div className={`relative aspect-[16/10] overflow-hidden rounded-xl ${accentClasses[project.accent]}`}>
              <span className="absolute left-5 top-5 font-mono text-xs tracking-widest text-zinc-600 dark:text-zinc-400">PROJECT / {project.number}</span>
            ...
            <div className="pt-7">
              ...
              <div className="mt-7 flex gap-5 border-t border-zinc-200 pt-5 dark:border-zinc-800">
                ...
                <button type="button" disabled aria-label={`GitHub for ${project.title} (link unavailable)`} className="inline-flex cursor-default items-center gap-1.5 text-sm font-semibold text-zinc-400 dark:text-zinc-600">GitHub <Icon name="github" className="h-4 w-4" /></button>
              </div>
            </div>
          </article>
        ))}
